...
var piggyback_room := PiggybackRoom.create_mesh_room()
var players := {}
var local_player:
	get:
		if not piggyback_room.peer_id in players: return null
		return players[piggyback_room.peer_id]

func _init():
	piggyback_room.peer_joined.connect(self._on_peer_joined)
	piggyback_room.peer_left.connect(self._on_peer_left)

func _enter_tree():
	multiplayer.multiplayer_peer = piggyback_room

func _ready():
	_add_player(piggyback_room.peer_id, multiplayer.get_unique_id()) # Add ourself
...
# Peer callbacks
func _on_peer_joined(peer_rpc_id: int, peer: PiggybackPeer) -> void:
...
func _on_peer_left(_peer_rpc_id: int, peer: PiggybackPeer) -> void:
	_remove_player(peer.id)

func _on_peer_chat(message: String, peer: PiggybackPeer) -> void:
	$UI/chat_history.text += "\n%s: %s" % [peer.id, message]
	players[peer.id].set_message(message)

func _on_peer_secret(peer: PiggybackPeer) -> void:
...
func _on_line_edit_text_submitted(new_text) -> void:
	if new_text == "": return
	$UI/chat_input.text = ""
	$UI/chat_history.text += "\n%s (Me): %s" % [piggyback_room.peer_id, new_text]
	local_player.set_message(new_text)
	piggyback_room.send_event("chat", [new_text])

func _on_secret_button_pressed() -> void:
	piggyback_room.send_event("secret")
